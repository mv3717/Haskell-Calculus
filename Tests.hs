...
maclaurinTests
  = [ (UnApp Sin (Id "x"), 2, 2) ==> 2.0
    ...
    , (UnApp Cos (Id "x"), 1, 1)  ==> 1.0
    , (UnApp Neg (Id "x"), 3, 23)  ==> (-3)
    , (BinApp Mul (Id "x") (Id "x"), 3, 3)  ==> 9
    , (e1, 6, 2)  ==> 30
    ]
...
showExpTests
  = [ Val 42 ==> "42.0"
    ...
    , e6 ==> "log(((3.0*(x*x))+2.0))"
    , (diff e1 "x") ==> "((5.0*1.0)+(0.0*x))"
    , (diff e2 "x") ==> "((((x*1.0)+(1.0*x))+0.0)+-(0.0))"
    , (diff e3 "x") ==> "(1.0+-((((((y*0.0)+(0.0*y))*(((4.0*x)*y)+-((y*y))))+-((((((4.0*x)*0.0)+(((4.0*1.0)+(0.0*x))*y))+-(((y*0.0)+(0.0*y))))*(y*y))))/((((4.0*x)*y)+-((y*y)))*(((4.0*x)*y)+-((y*y)))))))"
    , (diff e4 "x") ==> "-(-((sin(x)*1.0)))"
    , (diff e5 "x") ==> "(cos((1.0+log((2.0*x))))*(0.0+(((2.0*1.0)+(0.0*x))/(2.0*x))))"
    , (diff e6 "x") ==> "((((3.0*((x*1.0)+(1.0*x)))+(0.0*(x*x)))+0.0)/((3.0*(x*x))+2.0))"
    ]
